...
def main():
    ...
    ds = tf.data.Dataset.from_tensor_slices(
        dict(true_pose=all_true_pose, true_betas=all_true_betas, true_trans=all_true_trans,
             ...
             pred_trans=all_pred_trans)).batch(FLAGS.batch_size)

    all_eval_results = {}
    for batch in spu.progressbar(ds, total=len(relpaths), step=FLAGS.batch_size):
        batch_eval_results = eval_batch(batch)
        batch_eval_results = tf.nest.map_structure(lambda x: x.numpy(), batch_eval_results)
        for k, v in batch_eval_results.items():
            all_eval_results.setdefault(k, []).append(v)
    for k, l in all_eval_results.items():
        all_eval_results[k] = np.concatenate(l, axis=0)


    mpjpe = np.mean(all_eval_results['per_joint_errs_rootrel'])
    ampjpe = np.mean(all_eval_results['per_joint_errs'])
    mpjpe_pa = np.mean(all_eval_results['per_joint_errs_pa'])
    mve = np.mean(all_eval_results['vertex_errs_rootrel'])
    amve = np.mean(all_eval_results['vertex_errs'])
    mve_pa = np.mean(all_eval_results['vertex_errs_pa'])
    ...
    minor_joint_ids = [
        x for x in range(all_eval_results['per_joint_errs'].shape[1])
        if x not in major_joint_ids]

    major_dist = all_eval_results['per_joint_errs_rootrel'][:, major_joint_ids]
    minor_dist = all_eval_results['per_joint_errs_rootrel'][:, minor_joint_ids]
    major_dist_abs = all_eval_results['per_joint_errs'][:, major_joint_ids]
    major_dist_pa = all_eval_results['per_joint_errs_pa'][:, major_joint_ids]

    ori_joint_ids = [0, 1, 2, 4, 5, 16, 17, 18, 19]
    mpjae = np.rad2deg(np.mean(all_eval_results['per_joint_ori_errs'][:, ori_joint_ids]))
    mpjae_pa = np.rad2deg(np.mean(all_eval_results['per_joint_ori_errs_pa'][:, ori_joint_ids]))
...
def eval_batch(batch):
    ...
    return dict(
        per_joint_errs=1000*per_joint_errs, per_joint_errs_rootrel=1000*per_joint_errs_rootrel,
        per_joint_errs_pa=1000*per_joint_errs_pa, vertex_errs=1000*vertex_errs,
        vertex_errs_rootrel=1000*vertex_errs_rootrel, vertex_errs_pa=1000*vertex_errs_pa,
        per_joint_ori_errs=per_joint_ori_errs, per_joint_ori_errs_pa=per_joint_ori_errs_pa)
