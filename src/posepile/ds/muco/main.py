...
@spu.picklecache('muco.pkl', min_time="2020-02-24T19:18:24")
def make_dataset():
    joint_info, selected_joints = make_joint_info()

    root_3dhp = f'{DATA_ROOT}/3dhp'
    root_muco = f'{DATA_ROOT}/muco'
    ...
    all_detections = spu.load_pickle(f'{root_muco}/yolov3_detections.pkl')
    ...
    matloader = functools.lru_cache(1024)(util.load_mat)
    ...
    with spu.ThrottledPool() as pool:
        for i_sample, people, detections, visible_boxes in zip(
                spu.progressbar(valid_indices), sample_info[valid_indices],
            ...
            for i_person, ((i_subject, i_seq, i_cam, i_frame), visible_box) in enumerate(
                ...
                coord_bbox = boxlib.expand(boxlib.intersection(
                    boxlib.bb_of_points(im_coords),
                    boxlib.full([2048, 2048])), 1.05)
                bbox = boxlib.intersection_vertical(visible_box, coord_bbox)
...
def make_efficient_example(ex, root_muco, i_person):
    ...
    expanded_bbox = boxlib.intersection(expanded_bbox, boxlib.full([2048, 2048]))
    ...
    if not (spu.is_file_newer(new_im_path, "2020-02-15T23:28:26")):
        im = improc.imread(f'{root_muco}/{image_relpath}')
        new_im = cameravision.reproject_image(im, ex.camera, new_camera, dst_shape, antialias_factor=4)
        spu.ensure_parent_dir_exists(new_im_path)
        imageio.imwrite(new_im_path, new_im, quality=95)

    new_bbox_topleft = cameravision.reproject_image_points(ex.bbox[:2], ex.camera, new_camera)
    new_bbox = np.concatenate([new_bbox_topleft, ex.bbox[2:] * scale_factor])

    if ex.mask is None:
        noext, ext = osp.splitext(image_relpath[:-4])
        noext = noext.replace('unaugmented_set_001/', '')
        mask = rlemasklib.decode(spu.load_pickle(f'{root_muco}/masks/{noext}.pkl'))
    else:
        mask = ex.mask
    ...
    if mask is False:
        ...
        new_mask = cameravision.reproject_mask(mask, ex.camera, new_camera, dst_shape)
        new_mask_encoded = rlemasklib.encode(new_mask)

    return ds3d.Pose3DExample(
        osp.relpath(new_im_path, DATA_ROOT), ex.world_coords.astype(np.float32),
...
def make_joint_info():
    ...
    edges = (
        'htop-head-neck-rcla-rsho-relb-rwri-rhan,'
        'neck-spi4-thor-spi2-spin-pelv-rhip-rkne-rank-rfoo-rtoe')
